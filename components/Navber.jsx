...
function Navber({ children }){
  ...
  return (
    <>
      ...
      <div
        className="navbar m-auto flex items-center justify-between text-white px-6 py-9 transition-all duration-200" ref={nav}
      >
        <div className="navbar-start">
          <a href="#" className="text-[1.7rem] font-bold"
            ><span className="text-gray-400">Aadel</span> Banat</a>
        </div>

        <div className="dropdown flex lg:hidden">
          <label className="btn btn-circle swap swap-rotate">
            {/* <!-- this hidden checkbox controls the state --> */}
            <input type="checkbox" value={showMenu} onChange={(e)=> handleMenu(e.target.checked)} />
          ...
          <ul className={`items-center gap-8 text-[1rem] font-medium flex flex-col lg:flex-row bg-white rounded-lg lg:bg-transparent absolute lg:static top-full right-0 sm:right-6 w-full sm:w-1/3 lg:w-fit py-8 lg:py-0 text-dark-300 lg:text-white transition-all duration-300  ${showMenu ? "active" : "unactive"} `} ref={menuBox}>
            ...
            >
              <Link
                ...
                href="/"
                className="after:content-['Introduce'] after:block after:text-[#4bffa5] group-hover:my-[-1.7rem] block transition-all after:transition-all ease-linear duration-200">Introduce</Link>
            </li>
            <li className="relative h-[1.7rem] overflow-hidden group">
              <Link
              
                href="/resume"
                className="after:content-['Resume'] after:block after:text-[#4bffa5] group-hover:my-[-1.7rem] block transition-all after:transition-all ease-linear duration-200">Resume</Link>
            </li>
            <li className="relative h-[1.7rem] overflow-hidden group">
              <Link
              
                href="/work"
                className="after:content-['Work'] after:block after:text-[#4bffa5] group-hover:my-[-1.7rem] block transition-all after:transition-all ease-linear duration-200">Work</Link>
            </li>
          </ul>
      </div>
    </header>
